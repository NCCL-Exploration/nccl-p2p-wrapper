...
int main()
{
    ...
    printf("NCCL Point-to-Point Test!\n");
    
    // Start group call for point-to-point operations
    NCCL_CHECK(ncclGroupStart());
    
    // Send from rank 0 to rank 0 (self-send for single GPU test)
    printf("Calling ncclSend...\n");
    NCCL_CHECK(ncclSend(sendbuf, size, ncclFloat, 0, comm, stream));
    
    printf("Calling ncclRecv...\n");
    NCCL_CHECK(ncclRecv(recvbuf, size, ncclFloat, 0, comm, stream));
